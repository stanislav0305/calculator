var calculatorModule = (function(){	
	...
	let _orderStepsHelper = {
		...
		saveSocketsCount(element, itemId, value){
			for(let iIdx=0; iIdx < element.items.length; iIdx++){
				let item = element.items[iIdx];
				if(item.id === itemId){
					item.enteredValue = value;
	...
	let getCalculationHelper = function() {
		return {
				...
				panelSizeMainParamsCalc: function(step){
					...
					enteredValuesList.forEach(function(enteredSizes, index){
						...
						|| enteredSizes.h === ''){
							...
							_calculationHelper.mainParams.areaSize = _calculationHelper.mainParams.areaSize + panelAreaSize;
							_calculationHelper.mainParams.perimeterSize =  _calculationHelper.mainParams.perimeterSize + (enteredSizes.w/1000 + enteredSizes.h/1000) * 2;
							...
							w = w <= 0 ? 1: w;
							_calculationHelper.mainParams.drillingCount = _calculationHelper.mainParams.drillingCount + (1 +  w);
						}
					});
				}
				/*,				
			...
			railsAndSocketsCalc:function(step){
				...
				result = result + countToPriceCalc(socketSelectedItem.price, socketSelectedItem.enteredValue);
			...
			mountingTypeCalc:function(step){
				...
				if (selectedItem !== undefined && selectedItem.itemType === 'glue'){
					...
					return _calculationHelper.mainParams.drillingCount * calculatorConstsHelper.SKINALI_ONE_HOLE_DRILLING_PRICE;
				}
			...
			panelSizeCalc: function(step){
				...
				enteredValuesList.forEach(function(enteredSizes, index){
					...
					|| enteredSizes.h === ''){
						...
						if (enteredSizes.hardening === true){
							hardeningTotal = hardeningTotal + panelAreaSize * calculatorConstsHelper.SKINALI_PANEL_HARDENING_PRICE
						}
					}
				});
				...
				let grindingTotal = _calculationHelper.mainParams.perimeterSize * calculatorConstsHelper.SKINALI_PANEL_GRINDING_PRICE;
	...
	let module = {
		...
		init:function(calculatorConstsModule, orderStepsModule, orderStepModule){
			...
			orderStep.init(_orderStepsHelper, orderSteps, _calculationHelper);
