	...
	let orderStepRender =
	{
		...
		renderRadioAppendSize:function(element){
			...
			if (element.items !== undefined){				
				...
				orderStepRender.renderPanelSizeElementRows(element, element.items[selectedItemIdx].title*1, _stepId);
				orderStepRender.radioSetEvents(element);
				orderStepRender.setPanelSizeContinueBtnEvent();
			}
		},
		...
		setRadioEventAppendSizeElements:function(elementId){
			...
			$(document).on('change', `input:radio[name^="${elementId}"]`, function (event) {
				...
				if (step !== null){
					...
					orderStepRender.removePanelSizeElementRows(radioId);
					orderStepRender.renderPanelSizeElementRows(element, panelCount*1, stepId);
		...
		setPanelSizeContinueBtnEvent: function(){
			$(document).on('click', `#${orderStepRender.panelSizeContinueBtnEvent}`, function () {
				//let stepId = $(this).attr('step-id');
		...
		removePanelSizeContinueBtnEventEvent: function(){
			$(document).off('click', `#${orderStepRender.panelSizeContinueBtnEvent}`);
		},
